...
async def protected_endpoint(current_user: User = Depends(UserAuthSecurity(TokenType.access))):
    return current_user


@router.get("/me/role", response_model=Optional[UserRoleOut])
async def protected_role_endpoint(current_user: User = Depends(UserAuthSecurity(TokenType.access))):
