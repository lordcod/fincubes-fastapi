...
async def upload_file(body: bytes, object_path: str):
    ...
    async with s3_session.session.put(url, data=body, headers=dict(request.headers)) as response:
...
async def delete_file(*, url: Optional[str] = None, object_path: Optional[str] = None):
    ...
    async with s3_session.session.delete(url, headers=dict(request.headers)) as response:
        if response.status >= 400:
            text = await response.text()
